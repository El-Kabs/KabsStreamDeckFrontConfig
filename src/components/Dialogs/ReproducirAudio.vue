<template>
  <vs-dialog v-model="active" color="#012340" width="100px" v-on:close="close">
    <h3 style="color: #f4f7f8">Reproducir Audio</h3>
    <div class="con-form" style="">
      ...
      <input id="audio" type="file" @change="audioSelected" hidden />
      <vs-button
        color="#3F618C"
        class="buttonDialog"
        @click="chooseAudio"
...
export default {
  ...
  methods: {
    ...
    async audioSelected(evt) {
      var pathAudio = evt["target"]["files"][0];
      let data = new FormData();
      data.append("file", pathAudio);
      await axios
        .post("http://localhost:5000/upload", data)
        .then((response) => response.data).then((resp) => {
            this.audio = resp["path"]
        })
    },
    chooseAudio() {
