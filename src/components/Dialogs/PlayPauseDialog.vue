<template>
  <vs-dialog v-model="active" color="#012340" width="100px" v-on:close="close">
    <h3 style="color: #f4f7f8">Opciones multimedia</h3>
    <div class="con-form" style="">
      ...
      <vs-select placeholder="Opción" v-model="opcion" style="margin-top: 15px">
        <vs-option
          v-for="index in opciones"
          :key="index"
          v-bind:label="index"
          v-bind:value="index"
        >
...
export default {
  data: () => ({
    ...
    imagen: "",
    opcion: "",
    opciones: ["playToggle", "prevTrack", "nextTrack", "volumenMute", "volumenUp", "volumenDown"]
  }),
  methods: {
    fileSelected(evt) {
